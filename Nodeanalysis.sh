#!/bin/bash
### Set the duration/intervals in seconds (10 minutes = 20 * interval) and variables for hostname and core count. ADJUST DURATION AS NEEDED
interval=30
...
###Starts recording as background task. ADJUST THE INTERVALS AS NEEDED.
(
for ((i=0; i<10; i++))
do
...
sleep 30
stress-ng --cpu "$(nproc)" --io 4 --hdd 1 --vm-bytes $(awk '/MemFree/{printf "%dn", $2 * 0.9;}' < /proc/meminfo)k --vm-keep -m 1 --timeout 180 &
wait
